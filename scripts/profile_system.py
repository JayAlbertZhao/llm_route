...
# Profiling Config
TARGET_URL = "http://localhost:8081/v1/chat/completions" # Use one backend directly
OUTPUT_FILE = "data/profiling_data.csv"
MODEL_NAME = "qwen-8b" # Must match start_vllm_cluster.sh
...
class Profiler:
    ...
    async def send_request(self, session, prompt, current_rps, active_reqs):
        payload = {
            ...
            "max_tokens": 128, # Keep generation short to focus on TTFT vs Load
            "stream": True
        }
...
if __name__ == "__main__":
    profiler = Profiler("data/processed_workload.jsonl")
